...
.factory('DbService', function($q, Animes, DebugService) {
  var _db;
  ...
  function refresh() {
    _db.destroy().then(function() {
      ...
      _db.info().then(function(res) {
        console.log('db document count: ' + res.doc_count);
        if (res.doc_count === 0) {
          return $q.when(_db.bulkDocs('https://hummingbird.me/api/v1/users/cyalins/library'));
        }
      });
    })
